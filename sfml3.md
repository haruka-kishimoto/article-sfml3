...
並行して開発されている次期マイナーバージョンであるSFML 2.6では以下のような劇的な変更が多い。
- [\#1769](https://github.com/SFML/SFML/pull/1769), ジオメトリシェーダのサポート（OpenGL 3.2+コンテキストが利用可能な場合）
...
// 値を指定しての構築にはユーザ定義リテラルまたはヘルパ関数を使う。
// 度とラジアンのいずれもfloatで表現するため、
// floatを引数に取るコンストラクタは意味論的にあいまいになってしまう。
// ただし内部的には値を度として扱っており、
// floatを引数に取るprivateなコンストラクタを持ち、他のメンバ関数の実装に使われている。
angle = 20._deg;
